...
import java.sql.*;
public class cnacelcruiser2 extends HttpServlet {
		...
		String relpath=new java.net.URL(request.getScheme(),request.getServerName(),request.getServerPort(),request.getContextPath()).toString();
  		try{
            ...
            if(!rs.next()) 	response.sendRedirect(relpath+"/Pages/Login.jsp");
            	...
            	ResultSet rs1 = st.executeQuery("select * from cruisebooking where reqno =  ("+r+") " );
                    if (rs1.next())  {
                    st.execute("delete from cruisebooking where reqno =  ("+r+") " );
