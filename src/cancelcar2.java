...
import java.sql.*;
public class cancelcar2 extends HttpServlet {
		...
		String relpath=new java.net.URL(request.getScheme(),request.getServerName(),request.getServerPort(),request.getContextPath()).toString();
  		try{
            ...
            if(!rs.next()) 	response.sendRedirect(relpath+"/Pages/Login.jsp");
            	...
            	ResultSet rs1 = st.executeQuery("select * from carbooking where reqno =  ("+r+") " );
                    if (rs1.next())  {
                    st.execute("delete from carbooking where reqno =  ("+r+") " );
					...
					out.println("</TD></TR></tABLE><SCRIPT>setFooter(\""+relpath+"\",\""+relpath+"/Images/Back.jpg\")</SCRIPT>");   					} else {
						...
						getServletContext().getRequestDispatcher("/Pages/Accomadation/CancelReservationCar2.jsp").forward(request,response);
					}
				}
            }catch(Exception e){
